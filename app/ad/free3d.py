...
from gpt.Getprompt import TextProcessing
...
def call():
    ...
    if request.method == 'POST':
        ...
        if get_type == "Diffusion":
            ...
            rb_image.save("./static/"+ID+"/generated_img.png",'png')
            return ID+"/generated_img.png"
            ...
            if not os.path.exists('./static/'+ID):
                os.makedirs('./static/'+ID)
            Img = Image.open('./static/'+ID+'/generated_img.png')
            ...
            model_name = request.form["model_name"]
            model = model_name.split('_')
            gp = TextProcessing(LLM_model = 'DeveloperSejin/Fine_Tuned_Flan-T5-large_For_Describe_Furniture',ner_model = 'DeveloperSejin/NER_for_furniture_3D_object_create',key='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' , model = model_name)
            if model[0] == 'gpt':
                answer = gp.getAnswer(prompt = text, model_name = model_name)
            else:
                answer = gp.getAnswer(prompt = text, model_name = model_name)
            print(answer)
